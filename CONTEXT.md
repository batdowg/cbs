...
- **Dashboard column chooser**:
  ...
  - `/materials` defaults now read **Order ID, Title, Status, Workshop start date, Client, Order type, Workshop, Latest arrival date, Workshop status**. Optional columns include **Processed — Digital**, **Processed — Physical**, **Bulk order receiver**, **Outline**, **Credits/Teams** (displayed as `<credits> / <teams>` with teams derived as two per credit), **Facilitator(s)**, **Learner list**, **Region**, and **Shipping location title**. The processed columns show the latest `yyyy-mm-dd HH:MM UTC` timestamp and processor when available.
