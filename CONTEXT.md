...
- Certificate templates resolve under `app/assets`, preferring canonical filenames `fncert_template_{paper}_{lang}.pdf` with a legacy fallback `fncert_{paper}_{lang}.pdf`; inputs normalize case (`A4`/`LETTER`, `en`/`pt-br`) and errors list attempted names alongside available PDFs.
...
- An optional details panel can be enabled per size with Left/Right placement. Each size stores a Size % (50–100) that scales the detail line font/spacing, and rendered lines follow a fixed order: Facilitators, Location, Dates, and Class days/Contact hours. Location and Dates suppress labels (Location → `City, ST` for US sessions or `City, Country` otherwise, falling back to the saved string when formatting data is incomplete). Class days and Contact hours share a single line with a bullet separator when both values are present; empty variables continue to be skipped without aborting generation.
