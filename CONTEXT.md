...
- **Dashboard column chooser**:
  - `/sessions` and `/materials` share the same keyboard-accessible column chooser. **ID** and **Title** stay locked in the first two positions; all other columns may be shown/hidden or reordered. Preferences persist in `localStorage` as `cbs.sessions.columns.<user_id>` / `cbs.materials.columns.<user_id>` (fallbacks omit the user id). Resetting clears only the stored layout; filters and sort order remain.
